...
public class DbConnect {
 ...
 /*   public static void add() throws SQLException {
    ...
    public boolean updateUserProfile(User user) {
    	String query="UPDATE users SET name = ?, contact = ?, city = ?, area = ?, job = ? WHERE email = ?";
    ...
    public boolean loginUser(String username, String password) {
        String getStored = "SELECT password, salt FROM users WHERE username = ?";
        try (Connection conn = getConnection();
            ...
            if (rs.next()) {
                byte[] storedPwd = rs.getBytes("password");
                byte[] salt = rs.getBytes("salt");
                if (passwordMatch(password, salt, storedPwd)) {
